...
pylab.show()
        ...
        Và như vậy chỉ có khoảng 0 hoặc 1 quan sát nằm ngoài whisker của một đồ thị whisker and box.
...
print(cor)
        ...
        \begin{enumerate}
            ...
            \begin{equation*}
                P(U_{(k)} \leq u) = C_{k-1}^n u^{k-1} (1-u)^{n-k}
            \end{equation*}

            Ta tính hàm mật độ $f(u) = \dfrac{d P(U_{(k)} \leq u)}{du}$:
